...
# Import both the class and old function
...
def plot_histogram(defects, label, color=None):
    ...
    h[0] = 0  # ignore zero-sized defects
    total = h.sum()
    if total == 0:
        print(f"Skipping plot for {label} (no defect data)")
        return
    binp = 0.5 * (bins[1:] + bins[:-1])
...
def run_radius(
    ...
    """
    Original function-based radius defect pipeline.
    """
    ...
    for lipid_type in lipid_types:
        ...
        output_files = process_frames(
            ...
            min_cutoff_distance=cutoff_distance,
            use_cutoff=apply_protein_cutoff,
        )
        ...
        for frame_idx in range(frame_start, frame_end + 1):
            ...
            if os.path.exists(processed_file_path):
                ...
                defects_up, defects_down = calculate_defects(u, combine=False)
    ...
    for lipid_type, color in zip(lipid_types, ['red', 'blue', 'green']):
        plot_histogram(processed_defects_up[lipid_type], f'{lipid_type} Up', color=color)
        plot_histogram(processed_defects_down[lipid_type], f'{lipid_type} Down', color=color)
    plt.legend()
    ...
    plt.figure(figsize=(8, 6))
    for lipid_type, color in zip(lipid_types, ['red', 'blue', 'green']):
        plot_histogram(processed_defects_combined[lipid_type], f'{lipid_type} Combined', color=color)
    plt.legend()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run radius-based defect analysis.")
    parser.add_argument('--input', required=True, help='Input directory with *_frame_X.gro files')
    parser.add_argument('--output', required=True, help='Output directory')
    parser.add_argument('--lipids', nargs='+', required=True, help='List of lipid prefixes (e.g. PLacyl TGacyl)')
    parser.add_argument('--start', type=int, required=True, help='Starting frame index')
    parser.add_argument('--end', type=int, required=True, help='Ending frame index')
    parser.add_argument('--protein-count', type=int, required=True, help='Number of protein atoms in each .gro file')
    parser.add_argument('--cutoff', type=float, default=1.5, help='Minimum cutoff distance to protein (default: 1.5 Å)')
    parser.add_argument('--no-cutoff', action='store_true', help='Disable distance cutoff filter')
    parser.add_argument('--use-class', action='store_true', help='Use RadiusDefectAnalyzer class-based runner')
